    ...
    <AnimatePresence>
      {open && (
        <motion.aside
          ...
          className="fixed right-0 top-0 z-40 h-screen w-full max-w-[420px] border-l border-slate-200 bg-white shadow-xl"
        >
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">AI Co-Pilot</h2>
                <p className="text-sm text-slate-500">Sky TV Retention Intelligence Agent</p>
              </div>
            ...
            <div className="flex-1 space-y-6 overflow-y-auto px-6 py-6">
              <section className="space-y-3 rounded-2xl border border-slate-200 bg-sky-50/60 p-5">
                <div className="flex items-start gap-3">
                  <div className="rounded-full bg-white p-2 text-sky-600">
                    <Lightbulb size={20} />
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold text-slate-900">Today’s retention priorities</h3>
                ...
                <div className="space-y-2">
                  {suggestions.map((item) => (
                    <button
                      key={item}
                      type="button"
                      className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-sm font-medium text-slate-600 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700"
                    >
                      {item}
                    </button>
                  ))}
                ...
                <div className="space-y-2">
                  {smartLinks.map((link) => (
                    <a
                      key={link.label}
                      href={link.href}
                      className="block rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-sky-600 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700"
                    >
                      {link.label}
                    </a>
                  ))}
              ...
              <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-5">
                <h3 className="text-sm font-semibold text-slate-900">Co-Pilot abilities</h3>
