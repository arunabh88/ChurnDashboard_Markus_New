...
import {
  ...
  newUsersOverlapData,
  newUsersSummaryMetrics,
  newUsersAiRecommendations,
  NEW_USERS_TRIGGERS,
  type NewUsersSubscriber,
} from '@/lib/data/newUsersTriggers';
...
export function NewUsersTriggersDrilldown({ onBack, navigationSource, onNavigateToDashboard, onCreatePlaybook }: NewUsersTriggersDrilldownProps) {
  ...
  const getSubscribersByTrigger = (trigger: string): NewUsersSubscriber[] => {
    return newUsersSubscribers.filter((sub) => sub.triggers.includes(trigger));
  };
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {Object.values(NEW_USERS_TRIGGERS).map((trigger) => (
              <span
                key={trigger}
        ...
        <div className="flex flex-wrap gap-3">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => onCreatePlaybook(Object.values(NEW_USERS_TRIGGERS))}
      ...
      >
        <div className="glass-card rounded-xl p-6 border border-sky-500/20">
          ...
          <p className="text-3xl font-bold text-white">{newUsersSummaryMetrics.totalAffected.toLocaleString()}</p>
        ...
        <div className="glass-card rounded-xl p-6 border border-sky-500/20">
          ...
          <p className="text-3xl font-bold text-white">{newUsersSummaryMetrics.avgChurnRisk}%</p>
        ...
        <div className="glass-card rounded-xl p-6 border border-sky-500/20">
          ...
          <p className="text-3xl font-bold text-white">{newUsersSummaryMetrics.revenueAtRisk}</p>
      ...
      <div className="space-y-4">
        {newUsersTriggerMetrics.map((metric, index) => {
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      {metric.affectedCount.toLocaleString()} subscribers ({((metric.affectedCount / newUsersSummaryMetrics.totalAffected) * 100).toFixed(0)}%)
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
      ...
      >
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {newUsersOverlapData.map((item, idx) => (
                <motion.tr
      ...
      >
        ...
        <div className="space-y-4">
          {newUsersAiRecommendations.map((rec, idx) => (
            <motion.div
              key={rec.action}
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
