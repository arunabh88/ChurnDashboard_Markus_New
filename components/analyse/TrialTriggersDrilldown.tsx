...
import {
  ...
  aiRecommendations,
  TRIAL_TRIGGERS,
  type TrialSubscriber,
} from '@/lib/data/trialTriggers';

interface TrialTriggersDrilldownProps {
  onBack: () => void;
...
export function TrialTriggersDrilldown({ onBack, navigationSource, onNavigateToDashboard, onCreatePlaybook }: TrialTriggersDrilldownProps) {
  const [expandedTriggers, setExpandedTriggers] = useState<Set<string>>(new Set([TRIAL_TRIGGERS.ONBOARDING]));
  ...
  const getSubscribersByTrigger = (trigger: string): TrialSubscriber[] => {
    return trialSubscribers.filter((sub) => sub.triggers.includes(trigger));
  };

  const getRiskBadge = (risk: number) => {
    if (risk >= 90) return 'bg-red-500/20 text-red-400 border-red-500/30';
    if (risk >= 80) return 'bg-orange-500/20 text-orange-400 border-orange-500/30';
    if (risk >= 70) return 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30';
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          {navigationSource === 'early-lifecycle' ? (
            ...
            <Breadcrumb
              items={[
                ...
                { label: 'Trial Triggers', isActive: true },
              ]}
            />
          ) : (
            <Breadcrumb
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Trial Users: Trigger Analysis</h1>
          <p className="text-gray-400 max-w-2xl">
            Drilling into: Onboarding confusion, Content misalignment, Device setup issues
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            {Object.values(TRIAL_TRIGGERS).map((trigger) => (
              <span
                key={trigger}
        ...
        <div className="flex flex-wrap gap-3">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => onCreatePlaybook(Object.values(TRIAL_TRIGGERS))}
      ...
      >
        <div className="glass-card rounded-xl p-6 border border-sky-500/20">
          ...
          <p className="text-3xl font-bold text-white">{summaryMetrics.totalAffected.toLocaleString()}</p>
        ...
        <div className="glass-card rounded-xl p-6 border border-sky-500/20">
          ...
          <p className="text-3xl font-bold text-white">{summaryMetrics.avgChurnRisk}%</p>
        ...
        <div className="glass-card rounded-xl p-6 border border-sky-500/20">
          ...
          <p className="text-3xl font-bold text-white">{summaryMetrics.revenueAtRisk}</p>
      ...
      <div className="space-y-4">
        {triggerMetrics.map((metric, index) => {
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      {metric.affectedCount.toLocaleString()} subscribers ({((metric.affectedCount / summaryMetrics.totalAffected) * 100).toFixed(0)}%)
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
